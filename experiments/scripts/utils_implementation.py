    ...
    with tf.Session() as sess:    
    	...
    	for step in range(int(num_epochs * dataset.train_size) // train_batch_size):
            ...
            if step % eval_frequency == 0:
                ...
                loss_value, lr, predictions = sess.run([loss, learning_rate_var, train_prediction],
                                              feed_dict=feed_dict)
                minibatch_error = error_rate(predictions, batch_labels)
                validation_error = error_rate(
                        eval_in_batches(validation_data, sess, eval_batch_size,
                                        eval_data, eval_prediction),
                        validation_labels
                )
                train_metrics.append((loss_value, lr, minibatch_error, validation_error))
